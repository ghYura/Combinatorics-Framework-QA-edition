...
| 8b | **ML/LLM eval — two REAL local surrogates (differential)** | `ml_eval_surrogate/` | MODEL A/B × style × temp, queries `advanced_surrogate{,2}` (sklearn, OpenAI-compatible) over local HTTP | 12 PASS, real latency/tokens, formal **Pareto 2** (A vs B) |
