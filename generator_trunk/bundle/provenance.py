...
OWNER_ATTESTED_ENGINE = "owner-attested-java-engine"
...
_PATH_CATEGORIES: "tuple[tuple[str, str, str], ...]" = (
    # The owner's attestation covers the Java engine: Core, Reader, Executor and
    # Analyzer. Analyzer_trunk was missing from this list, so ~every file under
    # it fell to UNKNOWN -- a residual bucket whose reason text says "resolve
    # from owner records before publication". That under-stated the owner's
    # authorship of code he wrote, which is the opposite of the error this
    # module exists to prevent.
    ("Core_trunk", OWNER_ATTESTED_ENGINE,
     "covered by the owner's Java engine authorship attestation (Core, Reader, Executor, "
     "Analyzer); this is an attestation scope marker, not independent file-level corroboration"),
    ...
    ("Executor_trunk", OWNER_ATTESTED_ENGINE,
     "covered by the owner's Java engine authorship attestation (Core, Reader, Executor, "
     "Analyzer); this is an attestation scope marker, not independent file-level corroboration"),
    ("Analyzer_trunk", OWNER_ATTESTED_ENGINE,
     "covered by the owner's Java engine authorship attestation (Core, Reader, Executor, "
     "Analyzer); this is an attestation scope marker, not independent file-level corroboration"),
...
def file_provenance() -> dict:
    ...
    return {
        ...
        "caveat": (
            ...
            f"authorship. {OWNER_ATTESTED_ENGINE!r} records the scope of the owner's attestation, "
