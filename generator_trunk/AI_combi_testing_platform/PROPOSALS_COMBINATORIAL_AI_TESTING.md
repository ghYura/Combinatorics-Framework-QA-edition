...
❯ Is continuation of development of the Bundle (see code folder level up and one more) worth for AI testing purposes?                                                                                                         
  
● Good question, and it deserves an honest answer rather than a supportive one. Let me look at the actual engine
  rather than judge from the AI derivative.
...
Short answer: yes for the control plane, no for the Core — and AI testing can't be the justification for either. 
Here's the evidence behind that.
  The scale mismatch is ~5 orders of magnitude
  ┌──────────────────────────────────────────────┬─────────────────┐
  │                                              │   throughput    │
  ├──────────────────────────────────────────────┼─────────────────┤
  │ Bundle generator (measured, doc 13)          │ 80,157 rows/s   │
  ├──────────────────────────────────────────────┼─────────────────┤
  │ Bundle Core                                  │ 1,023 rows/s    │
  ├──────────────────────────────────────────────┼─────────────────┤
  │ Benchmark profiles                           │ 10K / 1M / 10M  │
  ├──────────────────────────────────────────────┼─────────────────┤
  │ AI target, measured (108 requests, 29.3 min) │ 0.06 requests/s │
  └──────────────────────────────────────────────┴─────────────────┘

  The engine is optimized for the axis AI testing doesn't use. Worse, AI testing needs the inverse competency: not
  "largest feasible design" but smallest honest design — maximum evidence per API call. That's the
  covering-array/screening/SPRT agenda, and the engine's whole benchmark story measures the opposite thing.
  nwise_optimal already exists in fwgen.py and is not on the AI path.

  The candidate model doesn't fit either

  Bundle candidate = executable program string → Reader distributes → Executor sandboxes → FW_VAR. The sandboxing
  exists because candidates are code.

  AI candidate = a prompt over HTTPS. The risk is on the far side of the boundary.

  Look at what the AI scenarios actually generate. Every TAIL is:

  _RESULT = run_candidate(PLAN)
  FW_VAR = _RESULT.code

  and every sheet is PLAN.schema = "json". So Core+XLSX+Postgres+Reader+Executor produces N Python stubs differing
  by attribute assignments — a 21,604-line Java engine computing itertools.product over five binary axes. 00_smoke =
  2⁵ = 32.

  Two gaps that matter more than any feature

  - run_model_k3.py — cited in FINDINGS §6 as the reproduction command — is not in the tree (not tracked, not
  present).
  - adapters/browser.py (406 lines), the only transport that ever produced real model data, is not in registry.py, 
  not in config.example.json, not referenced by any scenario. It's an orphan.

  So the project's single real-model result is currently not reproducible, and the code that produced it bypassed
  the engine it exists to demonstrate. For a project whose entire pitch is checkability, that costs more than a
  missing feature would.

  What genuinely fits, and is hard to replace

  - --repeat K --repeat-policy {disperse,local,nested} --repeat-scope — first-class repeats with a dispersion 
  policy. Given measured instability of 0.861 this is the most relevant thing in the engine, and almost nothing else
  in either ecosystem has it.
  - --budget-requests, --budget-monetary-cost, --cost-per-candidate — money and request count as planning-time 
  gates. Purpose-built for API-bounded work.
  - plan side-effect-free with exact cardinality before spending anything.
  - The constraint sieve; the Analyzer's formal/exploratory split and Pareto goals; the provenance and architecture
  gates.

  All control plane. None of it is Core.

  The market gap is real

  Eval frameworks (promptfoo, DeepEval, Inspect, Braintrust, HELM) have adapters and reporting but no design theory
  — their matrix is flat, with no strength and no denominator. CIT tools (ACTS, PICT, CAgen) have covering arrays
  and constraints but no LLM execution, no stochastic repeats, no evidence chain. Nobody sits in the middle. But the
  thing that would occupy that middle is ~3–5k lines, not 168k.

  Recommendation
  
  Separate three questions that are currently fused:

  1. Is the Bundle worth continuing? Probably — but decide that on the conventional-software-at-scale mission (the
  72M-combination fintech/telemetry work), where the throughput is actually consumed. AI testing cannot answer this
  question.
  2. Is it worth continuing for AI testing? Control plane emphatically yes; Core/Reader/Executor no. AI testing will
  never stress them, so it can never justify them.
  3. Is the AI platform worth continuing? Yes — it's the more valuable asset here. But it should stop routing
  through the heavy path.

  Concretely: declare a direct AI path — spec → plan/count → covering array → candidates → adapter → oracle →
  metrics → Analyzer, pure Python, no XLSX/Core/Reader/Executor. Keep the contracts (metrics KV, budgets, repeat
  policy, provenance hashes, Analyzer input); drop the transport. Doc 30 already permits this; it just hasn't been
  done. Then spend engine effort on the three things AI testing needs and the engine lacks — FW_Cover(t),
  distribution-aware repeats (the Analyzer must read K>1 as a distribution, not as more rows), and sequence covering
  arrays. All three are control plane.

  The test I'd apply from here
  
  Ask, each time: if AI testing vanished tomorrow, would the Core lose a requirement? Today the answer is no — every
  Core feature the AI platform touches is already exercised by engine_demo and the conventional scenarios. That
  asymmetry is the whole answer. AI testing is a consumer of this engine, never a driver of it, which makes it a
  good showcase and a bad justification.

  And in reverse: does AI testing need something the engine can't do? Yes — covering arrays, sequence covering
  arrays, distributional repeats. That's the list worth funding.

  One last note: 8,157 lines is expensive for a showcase. If the AI platform is a product line, that's fine. If it's
  a demo of the engine, it's several times too big — and its size is itself evidence that AI testing has
  requirements that don't reduce to the engine's.

  Want me to write this up as a decision memo next to the proposals doc?
